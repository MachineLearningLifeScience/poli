...
def get_gfp_base_seq():
    ...
    f = open(os.path.join(os.path.dirname(__file__), "ref", "data", "avGFP_reference_sequence.fa"))
    lines = f.readlines()
    f.close()
    seq = lines[1].strip()
    return seq
...
def get_experimental_X_y(random_state=1, train_size=5000, return_test=False, return_all=False, prefix=""):
...
def get_gfp_X_y_aa(data_df, large_only=False, ignore_stops=True, return_str=False):
    ...
    seqs.reset_index()
    #X = convert_aas_to_idx_array(seqs)
    X = np.array(seqs)
