...
def test_lambda_black_box_works_without_custom_info():
    def f_(x: NDArray[np.str_]) -> NDArray[np.float64]:
    ...
    assert (f(np.array([["a"] for _ in range(10)])) == 0.0).all()
...
def test_attaching_observer_to_lambda_black_box_works():
    ...
    f(np.array([["a"] for _ in range(10)]))

    assert (np.array(observer.results[0]["x"]) == "a").all()
